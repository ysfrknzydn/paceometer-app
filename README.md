...
- An always-on speed limit sign (top right of the live dashboard, styled like a real road sign) showing the posted limit near you whenever it's known — the same lookup the fourth zone state uses, just visible directly instead of only implied by a color.
