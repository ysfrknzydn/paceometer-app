...
- Testing (dev-only, doesn't touch the shipped app): `js/math/*.test.js` via Node's built-in test runner, and an independent Python port under `python/` via pytest, both checked against the same `tests/golden_vectors/`. Runs on every push/PR via `.github/workflows/tests.yml`. A separate checked-in Playwright suite (`tests/e2e/`, 2026-08-06, own scoped `package.json`) screenshots every screen/state — auth, dashboard, all 4 zone states, trip summary, settings, error states — in both light/dark and portrait/landscape, run manually (not CI-gated, since it's for visual review). See `docs/CLAUDE.md`.
