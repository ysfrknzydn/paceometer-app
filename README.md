...
- The same "Dev tools" toggle also holds four visual comparison pickers used to explore a final look before committing to one: color theme (18 palettes), font (27 options), color intensity (0–5), and rendering mode (default/neon/pastel/light/metallic/glass/retro terminal). Nothing here is final — see `TODO.md` for the still-open decision and `CLAUDE.md` for what to remove once one is picked.
